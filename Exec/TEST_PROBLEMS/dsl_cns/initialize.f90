module initialize_module
  ...
  public :: initialize_with_adaptive_grids
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine initialize_with_adaptive_grids(&
    ...
    logical       , intent(in   ) :: pmask(:)
    real(dp_t)    , pointer       :: dx(:,:)
    type(multifab), pointer       :: uold(:),wold(:)
    type(bc_tower), intent(  out) :: the_bc_tower
    ...
    ! allocate states
    allocate(uold(max_levs),wold(max_levs))
    ...
    ! multilevel_initialization: if (max_levs > 1) then
    ...
    ! end if multilevel_initialization  ! end if (maxlev > 1)
    ...
    do n = 1,nlevs
       ...
       call multifab_build(wold(n), mla%la(n), n_comp, ng_s)

       call setval(uold(n), ZERO, all=.true.)
       call setval(wold(n), ZERO, all=.true.)
    end do

    ! Initialize scalar and velocity data
    call initveldata   (uold,wold,dx,the_bc_tower%bc_tower_array,mla,prob_lo)
    !call initscalardata(sold,dx,the_bc_tower%bc_tower_array,mla,prob_lo)
